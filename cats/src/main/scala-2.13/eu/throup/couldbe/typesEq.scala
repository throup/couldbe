...
trait EqHierarchy[A] extends Hash[A] with PartialOrder[A]
object EqHierarchy {
  implicit def implicitEqCouldBeGiven[A: HashOrHashCode : PartialOrderOrEq]: EqHierarchy[CouldBeGiven[A]] =
  ...
  implicit def implicitEqIsGiven[A: HashOrHashCode: PartialOrderOrEq]: EqHierarchy[IsGiven[A]] =
